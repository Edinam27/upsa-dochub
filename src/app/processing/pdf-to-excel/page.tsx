...
import { ArrowLeft, FileSpreadsheet, Download, Upload, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
import Link from 'next/link';
import { cn } from '@/lib/utils';
import PDFToExcelTool from '@/components/tools/PDFToExcelTool';
import { createPDFProcessor } from '@/lib/pdf-processors';

export default function PDFToExcelProcessingPage() {
  ...
  const handleStartProcessing = async (files: File[], options?: any) => {
    ...
    try {
      const processor = createPDFProcessor('pdf-to-excel', options);
      if (!processor) {
        throw new Error('Processor not found for pdf-to-excel');
      }
      ...
      if (results.length > 0) {
        ...
        results.forEach((processedFile: any) => {
          ...
          if (url) {
            ...
            a.href = url;
            a.download = processedFile.name || `converted-${Date.now()}.xlsx`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <h1 className="text-xl font-semibold text-gray-900">PDF to Excel Converter</h1>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center space-x-3">
              {processingStatus === 'processing' && (
                <>
                  ...
                  <span className="text-blue-800 font-medium">Converting PDF to Excel...</span>
                </>
              )}
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="p-6">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-2">PDF to Excel Spreadsheet Converter</h2>
              <p className="text-gray-600">
                Convert PDF tables and data into editable Excel spreadsheets with intelligent table detection
              </p>
            </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <FileSpreadsheet className="h-8 w-8 text-emerald-600 mb-3" />
            <h3 className="font-semibold text-gray-900 mb-2">Table Detection</h3>
            <p className="text-gray-600 text-sm">
              Automatically identifies tables and preserves data structure in Excel format.
            </p>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <Download className="h-8 w-8 text-teal-600 mb-3" />
            <h3 className="font-semibold text-gray-900 mb-2">Instant Download</h3>
            <p className="text-gray-600 text-sm">
              Download your converted Excel spreadsheets immediately after processing.
            </p>
          </div>
        </div>
      </div>
