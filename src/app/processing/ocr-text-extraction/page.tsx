...
export default function OCRTextExtractionProcessingPage() {
  ...
  const handleStartProcessing = async (files: File[], options?: any) => {
    ...
    try {
      ...
      formData.append('toolId', 'pdf-ocr');
      if (options) {
      ...
      if (result.data && result.data.length > 0) {
        ...
        const textContent = result.data.map((file: any) => {
        ...
        result.data.forEach((processedFile: any) => {
          ...
          if (typeof processedFile.data === 'string') {
            blob = new Blob([processedFile.data], { type: 'text/plain' });
          } else {
            const uint8Array = new Uint8Array(processedFile.data);
            blob = new Blob([uint8Array], { type: processedFile.type || 'text/plain' });
          }
