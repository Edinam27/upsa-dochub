...
import { ArrowLeft, Presentation, Download, Upload, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
import Link from 'next/link';
import { cn } from '@/lib/utils';
import PDFToPPTTool from '@/components/tools/PDFToPPTTool';
import { createPDFProcessor } from '@/lib/pdf-processors';

export default function PDFToPPTProcessingPage() {
  ...
  const handleStartProcessing = async (files: File[], options?: any) => {
    ...
    try {
      const processor = createPDFProcessor('pdf-to-ppt', options);
      if (!processor) {
        throw new Error('Processor not found for pdf-to-ppt');
      }
      ...
      if (results.length > 0) {
        ...
        results.forEach((processedFile: any) => {
          ...
          if (url) {
            ...
            a.href = url;
            a.download = processedFile.name || `converted-${Date.now()}.pptx`;
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <h1 className="text-xl font-semibold text-gray-900">PDF to PowerPoint Converter</h1>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center space-x-3">
              {processingStatus === 'processing' && (
                <>
                  ...
                  <span className="text-blue-800 font-medium">Converting PDF to PowerPoint...</span>
                </>
              )}
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="p-6">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-2">PDF to PowerPoint Presentation Converter</h2>
              <p className="text-gray-600">
                Convert PDF documents to editable PowerPoint slides, preserving text, images, and layout
              </p>
            </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <p className="text-gray-600 text-sm">
              Simply drag and drop your PDF files or click to browse and select them.
            </p>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <Presentation className="h-8 w-8 text-red-600 mb-3" />
            <h3 className="font-semibold text-gray-900 mb-2">Layout Preservation</h3>
            <p className="text-gray-600 text-sm">
              Intelligently maps PDF pages to PowerPoint slides with correct positioning.
            </p>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <Download className="h-8 w-8 text-yellow-600 mb-3" />
            <h3 className="font-semibold text-gray-900 mb-2">Instant Download</h3>
            <p className="text-gray-600 text-sm">
              Download your converted PowerPoint presentations immediately after processing.
            </p>
          </div>
        </div>
      </div>
